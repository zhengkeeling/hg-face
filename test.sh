...
if [ "$MODE_CHOICE" = "1" ]; then
    ...
    if [ "$SETUP_KEEP_ALIVE" = "y" ] || [ "$SETUP_KEEP_ALIVE" = "Y" ]; then
        read -p "请输入保活URL (留空默认使用 https://www.google.com): " KEEP_ALIVE_URL_INPUT
        if [ -z "$KEEP_ALIVE_URL_INPUT" ]; then
            KEEP_ALIVE_URL="https://www.google.com"
            echo -e "${GREEN}使用默认保活URL: $KEEP_ALIVE_URL${NC}"
        else
            if [[ "$KEEP_ALIVE_URL_INPUT" != http* ]]; then
                echo -e "${RED}错误：URL格式不正确，将使用默认URL。${NC}"
                KEEP_ALIVE_URL="https://www.google.com"
            else
...
if [ -n "$KEEP_ALIVE_URL" ]; then
    ...
    echo "    curl -s --connect-timeout 15 -o /dev/null \"$KEEP_ALIVE_URL\"" >> keep_alive_task.sh
    echo "    if [ \$? -eq 0 ]; then" >> keep_alive_task.sh
    echo "        echo \"保活成功 - \$(date '+%Y-%m-%d %H:%M:%S')\" > keep_alive_status.log" >> keep_alive_task.sh
    echo "    else" >> keep_alive_task.sh
    echo "        echo \"保活失败 - \$(date '+%Y-%m-%d %H:%M:%S')\" > keep_alive_status.log" >> keep_alive_task.sh
...
# 退出脚本，避免重复执行
exit 0    if [ -f "$NODE_INFO_FILE" ]; then
        echo
        echo -e "${GREEN}========================================${NC}"
        echo -e "${GREEN}                      节点信息查看                      ${NC}"
        echo -e "${GREEN}========================================${NC}"
