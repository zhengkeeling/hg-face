...
echo -e "${BLUE}优化: 提升效率、增强隐私、支持更多流量分流${NC}"
echo
...
# 默认配置（你的信息）
DEFAULT_UUID="c10a3483-5de5-4416-9a37-a6c702b916ac"
DEFAULT_ARGO_DOMAIN="face.keeling.dpdns.org"
DEFAULT_ARGO_AUTH='{"AccountTag":"46fad1b6b0e334ca8ad9ea7ec29c4ddb","TunnelSecret":"J2TOKaJiWL8rph+m7iTfEOthVtREnhuvfWoHp4SmOog=","TunnelID":"29e3716e-783c-4a1f-9538-d40fa766006f","Endpoint":""}'
DEFAULT_HF_TOKEN="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"
DEFAULT_HF_REPO_ID="sukikeeling/face"
DEFAULT_HF_REPO_TYPE="spaces"

KEEP_ALIVE_HF="true"

configure_hf_keep_alive() {
    echo -e "${YELLOW}是否自定义Hugging Face保活配置? (默认: $DEFAULT_HF_REPO_ID, $DEFAULT_HF_REPO_TYPE) (y/n)${NC}"
    read -p "> " SETUP_KEEP_ALIVE
    if [ "$SETUP_KEEP_ALIVE" = "y" ] || [ "$SETUP_KEEP_ALIVE" = "Y" ]; then
        echo -e "${YELLOW}请输入Hugging Face Token (默认: $DEFAULT_HF_TOKEN):${NC}"
        read -sp "Token: " HF_TOKEN_INPUT
        echo
        HF_TOKEN="${HF_TOKEN_INPUT:-$DEFAULT_HF_TOKEN}"
        echo -e "${YELLOW}请输入仓库ID (默认: $DEFAULT_HF_REPO_ID):${NC}"
        read -p "Repo ID: " HF_REPO_ID_INPUT
        HF_REPO_ID="${HF_REPO_ID_INPUT:-$DEFAULT_HF_REPO_ID}"
        echo -e "${YELLOW}仓库类型 (默认: $DEFAULT_HF_REPO_TYPE):${NC}"
        read -p "Type: " HF_REPO_TYPE_INPUT
        HF_REPO_TYPE="${HF_REPO_TYPE_INPUT:-$DEFAULT_HF_REPO_TYPE}"
        echo -e "${GREEN}保活已设置为: $HF_REPO_ID ($HF_REPO_TYPE)${NC}"
    else
        HF_TOKEN="$DEFAULT_HF_TOKEN"
        HF_REPO_ID="$DEFAULT_HF_REPO_ID"
        HF_REPO_TYPE="$DEFAULT_HF_REPO_TYPE"
        echo -e "${GREEN}使用默认保活配置: $HF_REPO_ID ($HF_REPO_TYPE)${NC}"
    fi
}
...
if [ "$MODE_CHOICE" = "1" ]; then
    ...
    read -p "请输入UUID (默认: $DEFAULT_UUID): " UUID_INPUT
    UUID_INPUT="${UUID_INPUT:-$DEFAULT_UUID}"
    sed -i "s/UUID = os.environ.get('UUID', '[^']*')/UUID = os.environ.get('UUID', '$UUID_INPUT')/" app.py
    echo -e "${GREEN}UUID: $UUID_INPUT${NC}"
    
    sed -i "s/CFIP = os.environ.get('CFIP', '[^']*')/CFIP = os.environ.get('CFIP', 'joeyblog.net')/" app.py
    echo -e "${GREEN}优选IP: joeyblog.net${NC}"
    
    sed -i "s/ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', '[^']*')/ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', '$DEFAULT_ARGO_DOMAIN')/" app.py
    sed -i "s|ARGO_AUTH = os.environ.get('ARGO_AUTH', '[^']*')|ARGO_AUTH = os.environ.get('ARGO_AUTH', '$DEFAULT_ARGO_AUTH')|" app.py
    echo -e "${GREEN}Argo域名: $DEFAULT_ARGO_DOMAIN${NC}"
    ...
    if [ -n "$NAME_INPUT" ]; then
        ...
        echo -e "${GREEN}节点名称: $NAME_INPUT${NC}"
    fi
    ...
    echo -e "${GREEN}优选IP: $CFIP_INPUT${NC}"
    ...
    if [ -n "$CFPORT_INPUT" ]; then
        ...
        echo -e "${GREEN}优选端口: $CFPORT_INPUT${NC}"
    fi
    
    echo -e "${YELLOW}当前Argo域名: $(grep "ARGO_DOMAIN = " app.py | cut -d"'" -f4)${NC}"
    read -p "请输入Argo域名 (默认: $DEFAULT_ARGO_DOMAIN): " ARGO_DOMAIN_INPUT
    ARGO_DOMAIN_INPUT="${ARGO_DOMAIN_INPUT:-$DEFAULT_ARGO_DOMAIN}"
    sed -i "s|ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', '[^']*')|ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', '$ARGO_DOMAIN_INPUT')|" app.py
    echo -e "${GREEN}Argo域名: $ARGO_DOMAIN_INPUT${NC}"
    
    read -p "请输入Argo密钥 (默认: $DEFAULT_ARGO_AUTH): " ARGO_AUTH_INPUT
    ARGO_AUTH_INPUT="${ARGO_AUTH_INPUT:-$DEFAULT_ARGO_AUTH}"
    sed -i "s|ARGO_AUTH = os.environ.get('ARGO_AUTH', '[^']*')|ARGO_AUTH = os.environ.get('ARGO_AUTH', '$ARGO_AUTH_INPUT')|" app.py
    echo -e "${GREEN}Argo密钥已设置${NC}"
    
    configure_hf_keep_alive
    echo -e "${GREEN}完整配置完成！${NC}"
fi

echo -e "${BLUE}正在应用扩展分流...${NC}"
...
if [ "$KEEP_ALIVE_HF" = "true" ]; then
    ...
    KEEPALIVE_PID=$!
    echo -e "${GREEN}保活任务启动 (PID: $KEEPALIVE_PID)${NC}"
fi
...
if [ -n "$KEEPALIVE_PID" ]; then
    echo -e "保活PID: ${BLUE}$KEEPALIVE_PID${NC}"
fi
...
重启服务: pkill -f \"python3 app.py\" && nohup python3 app.py > app.log 2>&1 &
查看保活: ps aux | grep keep_alive_task.sh
停止保活: pkill -f keep_alive_task.sh
=== 分流说明 ===
支持YouTube, Twitter/X, TikTok, Netflix等平台加速"
echo "$SAVE_INFO" > "$NODE_INFO_FILE"
echo -e "${GREEN}节点信息已保存到 $NODE_INFO_FILE${NC}"
echo -e "${YELLOW}使用选项3或 -v 参数查看${NC}"
echo
