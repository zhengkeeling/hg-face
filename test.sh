...
CFG_ARGO_AUTH_JSON_B64="eyJPY2NvdW50VGFnIjoiNDZmYWQxYjZiMGUzMzRjYThhZDllYTdlYzI5YzRkZGIiLCJUdW5uZWxTZWNyZXQiOiJKMlRPS2FKaVdMOHJwaCttN2lUZkVPdGhWdFJFcmh1dnZXb0hwNF initiativesJiwIlR1bm5lbElEIjoiMjllMzcxNmUtNzgzYy00YTFmLTk1MzgtZDQwZmE3NjYwMDZmIiwiRW5kcG9pbnQiOiIifQ=="
...
TUNNEL_ID=$(echo "$CFG_ARGO_AUTH_JSON" | grep -o '"TunnelID":"[^"]*' | grep -o '[^"]*$')
mkdir -p /home/user/.cloudflared/
echo "$CFG_ARGO_AUTH_JSON" > "/home/user/.cloudflared/${TUNNEL_ID}.json"
...
XRAY_PID=$!
sleep 2
nohup ./$S_CLOUDFLARED tunnel --no-autoupdate run --url http://127.0.0.1:${CFG_PORT} > argo.log 2>&1 &
ARGO_PID=$!
sleep 10
